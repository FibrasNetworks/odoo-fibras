...
user_o = 'odoo'  # el usuario de odoo origen
user_d = 'odoo'  # el usuario de odoo destino
pwd_o = 'odoo'  # contrasenia de usuario odoo origen
pwd_d = 'odoo'  # contrasenia de usuario destino
dbname_o = 'FibrasGestion'  # nombre de base de datos origen
dbname_d = 'gestionfn'  # nombre de base de datos destino
web_o = '10.0.1.17'  # ip o dir web del origen
web_d = '10.0.0.188'  # ip o dir web del destino
# datos de los modelos y campos
model_o = 'res_partner'  # nombre del modelo de origen
model_d = 'res_partner'  # nombre del modelo del destino
idant_o = 'x_id_anterior'  # nombre del campo a usar en el destino para almacenar id anterior, dejar vacio si no se usa
# Es necesario crear el campo x_id_anterior en el model_d para usarlo en la variable idant_o
condi1_o = [('active', '=', 'true'), ('id', '>',
                                      '10')]  # ('id', '>', '10') las primeras 10 id son reservadas de cada instalacion ('is_company','=','true') para las compañias primero, luego para los individuos ('is_company','!=','true')
...
campos = ['id', 'active', 'name', 'company_type', 'street', 'street2', 'city', 'state_id', 'responsability_id',
          ...
          'email', 'title', 'lang', 'comment', 'customer', 'supplier', 'notify_email', 'vat', 'document_type_id',
          'document_number', 'property_product_pricelist',
...
sock_common_o = xmlrpc.client.ServerProxy('http://gestion.fibras.com.ar/xmlrpc/common', context=gcontext)
uid_o = sock_common_o.login(dbname_o, user_o, pwd_o)

# Get the uid destino de los datos
sock_common_d = xmlrpc.client.ServerProxy('http://10.0.0.188_8069/xmlrpc/common', context=gcontext)
uid_d = sock_common_d.login(dbname_d, user_d, pwd_d)

# reemplazar el valor de la ip o url del servidor de origen con su puerto
sock_o = xmlrpc.client.ServerProxy('http://gestion.fibras.com.ar/xmlrpc/object', context=gcontext)

# reemplazar el valor de la ip o url del servidor de destino con su puerto
sock_d = xmlrpc.client.ServerProxy('http://10.0.0.188:8069/xmlrpc/object', context=gcontext)
...
for i in registro_ids_o:
    ...
    if property_product_pricelist_o[0] == 6:  # forzar precio gremio a 2 en la nueva
        ...
        property_product_pricelist_o = registro_data_o['property_product_pricelist']
    """
    property_product_pricelist_o[0] = 1
    ...
    # argentina es el 10 en odoo 11 y el 11 en odoo 8 y 9
    if country_id_o:
    ...
    if state_id_o:
        if state_id_o[
            0] == 67:  # 61 era buenos aires en odoo 8 sock_d.execute(dbname_d,uid_d,pwd_d,'res.country.state','search',[('name','=',property_product_pricelist_o[1])])
            state_id_o[0] = 565  # 554 es el caso de buenos aires en el odoo 11 que instale.
        else:
            state_id_o = [
                565]  # si no es de buenos aires lo dejo vacío para no cambiarle mal la provincia, mejor no tener el dato y preguntar en mi caso.
    else:
        state_id_o = [565]  # si no tengo el dato fuerzo BS AS
    ...
    valores_update = {
        ...
        'x_Plandepago': registro_data_o['x_Plandepago'],
        'x_MorosoNoCobrable': registro_data_o['x_td_visa'],
        'x_tc_visa': registro_data_o['x_tc_visa'],
        'x_cbu': registro_data_o['x_cbu'],
        'x_nro_cliente': registro_data_o['ref']
    }
